...
from specs.computers.student import set_targets as target_right_ships
from specs.computers.random import set_targets as target_left_ships
...
class Simulation:
  ...
  def check_finished(self, win=None):
    ...
    if self.round > 1000:
      if win:
        self.speed = 0
        win.addstr(1, 49, "{:>30}".format("DRAW (Timeout)"),
                   curses.color_pair(3))
      else:
        print("DRAW (Timeout)")
      self.finished = True
    ...
    if lScore == 0 and rScore == 0:
      if win:
        self.speed = 0
        win.addstr(1, 49, "{:>30}".format("DRAW (none alive)"),
                   curses.color_pair(3))
      else:
  ...
  def bootGUI(self, screen):
    ...
    win.addstr(
      23, 6,
      "Press a key from 0-9 to change simulation speed (ESC) exit combat")
    ...
    while True:
      ...
      # Combat continues for as long as both players have ships and we have not passed round 1000
      # the last condition is to prevent stalemates where no players can hit each other.
      if not self.finished and self.speed > 0 and frame > int(
          self.refreshrate / self.speed):
